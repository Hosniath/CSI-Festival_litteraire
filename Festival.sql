...
SELECT date_ debut, date fin INTO date_debut,date_fin
...
IF NEW.date_voeux NOT BETWEEN date_debut AND date_fin THEN
 RAISE EXCEPTION 'date non comprises dans la periode de la campagne' ;
END IF;
RETURN NEW;
END ;
$$ LANGUAGE plogsal;
...
BEFORE INSERT ON vœux
FOR EACH ROW
EXÉCUTE FUNCTION check_date();
